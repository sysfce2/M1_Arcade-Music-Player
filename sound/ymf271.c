/*
	Yamaha YMF271-F "OPX" emulator v1.0
	By R. Belmont and Ville Linde.
...
#ifndef PI
#define PI M_PI	// this works for Linux at least :-)
#endif
...
/*	if (slot->waveform != 7)
	{
		printf("YMF271 ERROR: Waveform %d in update_pcm !!!\n", slot->waveform);
	}*/
...
static void ymf271_w(int chipnum, int offset, int data)
{
	...
	switch (offset)
	{
		...
		case 1:
			...
			vgm_write(chip->vgm_idx, 0x00, chip->reg0, data);
			break;
		case 2:
			chip->reg1 = data;
			break;
		case 3:
			...
			vgm_write(chip->vgm_idx, 0x01, chip->reg1, data);
			break;
		case 4:
			chip->reg2 = data;
			break;
		case 5:
			...
			vgm_write(chip->vgm_idx, 0x02, chip->reg2, data);
			break;
		case 6:
			chip->reg3 = data;
			break;
		case 7:
			...
			vgm_write(chip->vgm_idx, 0x03, chip->reg3, data);
			break;
		case 8:
			chip->pcmreg = data;
			break;
		case 9:
			...
			vgm_write(chip->vgm_idx, 0x04, chip->pcmreg, data);
			break;
		case 0xc:
			chip->timerreg = data;
			break;
...
static void ymf271_init(int i, UINT8 *rom, UINT32 romsize, void (*cb)(int), read8_handler ext_read, write8_handler ext_write)
{
	...
	YMF271[i].vgm_idx = vgm_open(VGMC_YMF271, CLOCK);
	vgm_write_large_data(YMF271[i].vgm_idx, 0x01, romsize, 0x00, 0x00, YMF271[i].rom);
...
int YMF271_sh_start( const struct MachineSound *msound )
{
	...
	for(i=0; i<intf->num; i++)
	{
		...
		ymf271_init(i, memory_region(intf->region[0]), memory_region_length(intf->region[0]), intf->irq_callback[i], intf->ext_read, intf->ext_write);
