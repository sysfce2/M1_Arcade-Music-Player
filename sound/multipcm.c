...
static struct _MultiPCM mPCMs[2];
...
static void MultiPCM_update(int num, stream_sample_t **buffer, int length )
{
	struct _MultiPCM *ptChip = &mPCMs[num];
...
static void *multipcm_start(int sndindex, int clock, const void *config)
{
	...
	ptChip=&mPCMs[sndindex];

	ptChip->ROM=(signed char *)memory_region(intf->region[sndindex]); 
	ptChip->Rate=(float) clock / MULTIPCM_CLOCKDIV;

	sprintf(buf[0], "%s %d L", "MultiPCM", sndindex);
	sprintf(buf[1], "%s %d R", "MultiPCM", sndindex);
	name[0] = buf[0];
	name[1] = buf[1];
	vol[0]=intf->mixing_level[sndindex] >> 16;
	vol[1]=intf->mixing_level[sndindex] & 0xffff;
	ptChip->stream = stream_init_multi(2, name, vol, ptChip->Rate, sndindex, MultiPCM_update);

        ptChip->type = intf->type[sndindex];
	ptChip->banksize = intf->banksize[sndindex];
	...
	ptChip->vgm_idx = vgm_open(VGMC_MULTIPCM, clock);
	vgm_write_large_data(ptChip->vgm_idx, 0x01, memory_region_length(intf->region[sndindex]), 0x00, 0x00, ptChip->ROM);
...
int MultiPCM_sh_start(const struct MachineSound *msound)
{
	...
	{
		multipcm_start(chip, intf->clock[chip], intf);
	}
...
{
	struct _MultiPCM *ptChip = &mPCMs[chip];
	
	vgm_write(ptChip->vgm_idx, 0x00, offset, data);
	switch(offset)
	{
		case 0:		//Data write
			//vgm_write(ptChip->vgm_idx, 0x00, (ptChip->CurSlot << 3) | ptChip->Address, data);
			//WriteSlot(ptChip,ptChip->Slots+ptChip->CurSlot,ptChip->Address,data);
			WriteSlot(ptChip,ptChip->Slots+val2chan[ptChip->CurSlot],ptChip->Address,data);
			break;
		case 1:
			//ptChip->CurSlot=val2chan[data&0x1f];
			ptChip->CurSlot=data&0x1f;
			break;
...
{
	struct _MultiPCM *ptChip = &mPCMs[which];
	if (leftoffs == rightoffs)
	{
		vgm_write(ptChip->vgm_idx, 0x01, leftoffs >> 16, 0x01 | 0x02);
	}
	else
	{
		vgm_write(ptChip->vgm_idx, 0x01, leftoffs >> 16, 0x01);
		vgm_write(ptChip->vgm_idx, 0x01, rightoffs >> 16, 0x02);
	}
...
{
	if (mPCMs[0].type == MULTIPCM_MODE_STADCROSS)	// multi32 with mono bankswitching GAL
	{
		mPCMs[0].BankL = mPCMs[0].BankR = (data&0x7);
	}
	else if (mPCMs[0].type == MULTIPCM_MODE_MULTI32)	// multi32
	{
		mPCMs[0].BankL = (data>>3)&0x7;
		mPCMs[0].BankR = data & 0x7;
	}
	else
	{
		mPCMs[0].BankL = mPCMs[0].BankR = data&3;
	}

	mPCMs[0].BankL *= mPCMs[0].banksize;
	mPCMs[0].BankR *= mPCMs[0].banksize;
	if (mPCMs[0].BankL == mPCMs[0].BankR)
	{
		vgm_write(mPCMs[0].vgm_idx, 0x01, mPCMs[0].BankL >> 16, 0x01 | 0x02);
	}
	else
	{
		vgm_write(mPCMs[0].vgm_idx, 0x01, mPCMs[0].BankL >> 16, 0x01);
		vgm_write(mPCMs[0].vgm_idx, 0x01, mPCMs[0].BankR >> 16, 0x02);
	}
}			  
WRITE_HANDLER( MultiPCM_bank_1_w )
{
	if (mPCMs[1].type == MULTIPCM_MODE_STADCROSS)	// multi32 with mono bankswitching GAL
	{
		mPCMs[1].BankL = mPCMs[1].BankR = (data&0x7);
	}
	else if (mPCMs[1].type == MULTIPCM_MODE_MULTI32)	// multi32
	{
		mPCMs[1].BankL = (data>>3)&0x7;
		mPCMs[1].BankR = data & 0x7;
	}
	else
	{
		mPCMs[1].BankL = mPCMs[1].BankR = data&3;
	}

	mPCMs[1].BankL *= mPCMs[1].banksize;
	mPCMs[1].BankR *= mPCMs[1].banksize;
	if (mPCMs[1].BankL == mPCMs[1].BankR)
	{
		vgm_write(mPCMs[1].vgm_idx, 0x01, mPCMs[1].BankL >> 16, 0x01 | 0x02);
	}
	else
	{
		vgm_write(mPCMs[1].vgm_idx, 0x01, mPCMs[1].BankL >> 16, 0x01);
		vgm_write(mPCMs[1].vgm_idx, 0x01, mPCMs[1].BankR >> 16, 0x02);
	}
}
