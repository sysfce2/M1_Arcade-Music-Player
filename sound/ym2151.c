...
double ym2151_hack = 1.0f;
static int ym2151_comptab = 0;
...
/* this must be done _before_ a call to YM2151ResetChip() */
		...
		YMPSG->vgm_idx = vgm_open(VGMC_YM2151, YMPSG->clock);
	}
...
void YM2151UpdateOne(int num, INT16 **buffers, int length)
{
	...
	if (!ym2151_comptab)
	{
		...
		{
			/* ASG 980324: changed to compute both tim_A_tab and timer_A_time */
			pom= ( 64.0  *  (1024.0-i) / ym2151_chip->clock );
			#ifdef USE_MAME_TIMERS
				ym2151_chip->timer_A_time[i] = pom * ym2151_hack;
			#else
				ym2151_chip->tim_A_tab[i] = pom * (double)ym2151_chip->sampfreq * mult;  /* number of samples that timer period takes (fixed point) */
			#endif
		}
		for (i=0; i<256; i++)
		{
			/* ASG 980324: changed to compute both tim_B_tab and timer_B_time */
			pom= ( 1024.0 * (256.0-i)  / ym2151_chip->clock );
			#ifdef USE_MAME_TIMERS
				ym2151_chip->timer_B_time[i] = pom * ym2151_hack;
			#else
				ym2151_chip->tim_B_tab[i] = pom * (double)ym2151_chip->sampfreq * mult;  /* number of samples that timer period takes (fixed point) */
			#endif
		}
