...
static long find_sample(long adrs, long bank, int voice)
{
	...
	switch (banking_type)
	{
		...
		case C140_TYPE_ASIC219:
			...
			newadr = ((REG[asic219banks[voice/4]]&0x3) * 0x20000) + adrs;
			break;
	}
...
WRITE_HANDLER( C140_w )
{
	...
	if ((offset >= 0x1f8) && (banking_type == C140_TYPE_ASIC219))
	{
		offset -= 8;
	}
	REG[offset]=data;
	if( offset<0x180 )
	{
		...
		if( (offset&0xf)==0x5 )
		{
			if( data&0x80 )
			{
				...
				v->bank = vreg->bank;
				v->mode = data;
				if (banking_type == C140_TYPE_ASIC219)
				{
...
static void update_stereo(int ch, INT16 **buffer, int length)
{
	...
	voicecnt = (banking_type == C140_TYPE_ASIC219) ? 16 : 24;
	...
	for( i=0;i<voicecnt;i++ )
	{
		...
		if( v->key )
		{
			...
			pSampleData=(signed char*)((unsigned long)pRom + find_sample(st, v->bank, i));
			...
			/* Switch on data type */
			if ((v->mode&8) && (banking_type != C140_TYPE_ASIC219))
			{
...
int C140_sh_start( const struct MachineSound *msound )
{
	...
	if (pRom != NULL)
	{
		vgm_idx = vgm_open(VGMC_C140, baserate);
		vgm_header_set(vgm_idx, 0x01, banking_type);
		vgm_write_large_data(vgm_idx, 0x01, memory_region_length(intf->region), 0x00, 0x00, pRom);
	}
	else
	{
		printf("VGM Warning: C140 wants to use dynamic memory (i.e. RAM) - disabled C140 logging!\n");
		vgm_idx = 0xFFFF;
	}
