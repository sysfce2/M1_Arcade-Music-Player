...
int K051649_sh_start(const struct MachineSound *msound)
{
	...
	vgm_idx = vgm_open(VGMC_K051649, mclock);
...
{
	stream_update(stream,0);
	vgm_write(vgm_idx, 0x04, offset, data);
	channel_list[offset>>5].waveform[offset&0x1f]=data;
}
...
{
	stream_update(stream,0);
	vgm_write(vgm_idx, 0x02, offset, data);
...
WRITE_HANDLER( K051649_frequency_w )
{
	...
	stream_update(stream,0);
	vgm_write(vgm_idx, 0x01, offset, data);
...
{
	stream_update(stream,0);
	vgm_write(vgm_idx, 0x03, offset, data);
