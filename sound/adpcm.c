...
static UINT16 okim6295_vgm_idx[MAX_OKIM6295];
...
int OKIM6295_sh_start(const struct MachineSound *msound)
{
	...
	for (i = 0; i < num_voices; i++)
	{
		...
		if (! voice)
		{
			okim6295_vgm_idx[chip] = vgm_open(VGMC_OKIM6295, intf->frequency[chip] * 165);
			vgm_write_large_data(okim6295_vgm_idx[chip], 0x01, memory_region_length(intf->region[chip]), 0x00, 0x00, memory_region(intf->region[chip]));
		}
...
void OKIM6295_set_bank_base(int which, int base)
{
	...
	vgm_write(okim6295_vgm_idx[which], 0x00, 0x0F, (base >> 18) & 0xFF);
