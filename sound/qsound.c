...
#define CHANNELS ( 2 )
	...
	vgm_idx = vgm_open(VGMC_QSOUND, intf->clock);
	vgm_write_large_data(vgm_idx, 0x01, memory_region_length(intf->region), 0x00, 0x00, qsound_sample_rom);
...
{
	//vgm_write(vgm_idx, 0x00, 0x00, data);
	qsound_data=(qsound_data&0xff)|(data<<8);
}

WRITE_HANDLER( qsound_data_l_w )
{
	//vgm_write(vgm_idx, 0x01, 0x00, data);
	qsound_data=(qsound_data&0xff00)|data;
}

WRITE_HANDLER( qsound_cmd_w )
{
	//vgm_write(vgm_idx, 0x02, 0x00, data);
	vgm_write(vgm_idx, 0x00, qsound_data, data);
