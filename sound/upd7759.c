...
int UPD7759_sh_start(const struct MachineSound *msound)
{
	...
	for(chip=0; chip<intf->num; chip++) {
		...
		ch->vgm_idx = vgm_open(VGMC_UPD7759, 640000);
		if (intf->mode == UPD7759_SLAVE_MODE)
		{
			vgm_header_set(ch->vgm_idx, 0x00, 0x01);
		}
		else
		{
			vgm_header_set(ch->vgm_idx, 0x00, 0x00);
			vgm_write_large_data(ch->vgm_idx, 0x01, memory_region_length(intf->region[chip]), 0x00, 0x00, ch->rom);
		}
	}
	return 0;
}
...
void UPD7759_set_bank_base(int chip, UINT32 base)
{
	...
	vgm_write(ch->vgm_idx, 0x00, 0x03, base / 0x20000);
...
{
	vgm_write(UPD7759_chips.chip[0].vgm_idx, 0x00, 0x01, data);
	UPD7759_start_w(0,data);
}

WRITE_HANDLER(UPD7759_0_reset_w)
{
	vgm_write(UPD7759_chips.chip[0].vgm_idx, 0x00, 0x00, data);
	UPD7759_reset_w(0,data);
}

WRITE_HANDLER(UPD7759_0_port_w)
{
	vgm_write(UPD7759_chips.chip[0].vgm_idx, 0x00, 0x02, data);
