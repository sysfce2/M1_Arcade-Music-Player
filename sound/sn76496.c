...
static int SN76496_init(const struct MachineSound *msound,int chip,int clock,int volume,int sample_rate)
{
	...
	R->vgm_idx = vgm_open(VGMC_SN76496, clock);
	...
	vgm_header_set(R->vgm_idx, 0x01, 0x8000);
	vgm_header_set(R->vgm_idx, 0x02, 0x09);
	vgm_header_set(R->vgm_idx, 0x03, 0x00);
	vgm_header_set(R->vgm_idx, 0x04, 0x00);
	vgm_header_set(R->vgm_idx, 0x05, 0x00);
	//vgm_write(R->vgm_idx, 0x01, 0xFF, 0x00);	// not really needed
