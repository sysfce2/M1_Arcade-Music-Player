...
#if 1 // def VERBOSE
					printf("YMF278B: slot %2d wave %3d lfo=%d vib=%d ar=%d d1r=%d dl=%d d2r=%d rc=%d rr=%d am=%d\n", snum, slot->wave,
					  	 slot->lfo, slot->vib, slot->AR, slot->D1R, slot->DL, slot->D2R, slot->RC, slot->RR, slot->AM);
					printf("                  b=%d, start=%x, loop=%x, end=%x, oct=%d, fn=%d, step=%x\n", slot->bits, slot->startaddr, slot->loopaddr>>16, slot->endaddr>>16, oct, slot->FN, slot->step);
#endif
				}
				else
				{
#if 1 //def VERBOSE
					printf("YMF278B: slot %2d off\n", snum);
#endif
					if(slot->active)
					{
						slot->env_step = 4;
...
static void ymf278b_data_port_A_w(int num, UINT8 data)
{
	...
	vgm_write(YMF278B[num].vgm_idx, 0x00, YMF278B[num].port_A, data);
}
...
static void ymf278b_data_port_B_w(int num, UINT8 data)
{
	...
	vgm_write(YMF278B[num].vgm_idx, 0x01, YMF278B[num].port_B, data);
}
...
static void ymf278b_data_port_C_w(int num, UINT8 data)
{
	...
	vgm_write(YMF278B[num].vgm_idx, 0x030, YMF278B[num].port_C, data);
}

static void ymf278b_init(INT8 num, UINT8 *rom, UINT32 romsize, void (*cb)(int), int clock)
{
	...
	YMF278B[num].vgm_idx = vgm_open(VGMC_YMF278B, clock);
	vgm_write_large_data(YMF278B[num].vgm_idx, 0x01, romsize, 0x00, 0x00, YMF278B[num].rom);
}
...
int YMF278B_sh_start( const struct MachineSound *msound )
{
	...
	for(i=0; i<intf->num; i++)
	{
		...
		ymf278b_init(i, memory_region(intf->region[0]), memory_region_length(intf->region[0]), intf->irq_callback[i], intf->clock[i]);
