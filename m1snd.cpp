...
static void m1snd_restartgame(void)
{
	if (cur_board)
	{
	...
	switch (command)
	{
		...
		case M1_CMD_SONGJMP:	// jump to song #
			...
			if (!m1snd_isQueueBusy())
			{
				...
				{
					m1snd_restartgame();
					vgm_start(vgmlog, curgame, iparm);
				}
...
//			return mfgstrs[games[parm].mfg];
		...
		case M1_SINF_BNAME:
			// Note by Valley Bell: slightly modified to prevent crashing
			brd_bak = cur_board;
			cur_board = (BoardT *)NULL;
		...
		case M1_SINF_BHARDWARE:
			brd_bak = cur_board;	// same as above
			cur_board = (BoardT *)NULL;
